...
```{r}
#First we'll replace all the NAs with 0 because we're working in R
scene_joined[is.na(scene_joined)]<-0

#Plot the standard deviations and mean

scene_joined$mean<-rowMeans(scene_joined[,2:24], na.rm=TRUE)

scene_joined$sd<-apply(scene_joined[,2:24], MARGIN = 1, FUN=sd)
...
sharpe_hist<-ggplot(scene_joined, aes(Sharpe))+
  ...
  geom_vline(xintercept = 2, col="Blue", size=1.4)+
  theme_economist(base_size=15)

sharpe_hist

sharpe_scatter<-ggplot(scene_joined, aes(sd))+
  geom_point(aes(y=mean))+
  geom_abline(intercept=0, slope=2, size=1.4, col="Blue")+
...
#Let's create a subset of customers who are 'reliable spendes' and see if we can predict them easier
...
for (i in 1:length){
fitted_arima[i]<-as.numeric(models[[i]]$mean)
}

actuals$fitted<-fitted_arima
#Let's calculate the mean absolute error and RMSE
mean(abs(actuals$aug_2016-fitted_arima))
mean(actuals$aug_2016)

arima_residuals<-ggplot(actuals, aes(x=fitted))+
  geom_point(aes(y=aug_2016))+
...
#Adjusted MAPE because mape function wont work for observatiosn where actual is zero (undefined solution)
Adj_mape = mean(abs(actuals$aug_2016-fitted_arima))/mean(actuals$aug_2016)
print(Adj_mape)


rmse(actuals$aug_2016, fitted_arima)
