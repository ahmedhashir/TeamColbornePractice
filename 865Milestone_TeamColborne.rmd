...
# Spark script
scene_mbr_dim <- spark_read_csv(sc, 
                                name='scene_mbr_dim', 
                                path=paste(in_path_spark, 
                                           'scene_mbr_dim.csv', sep=""), 
                                header = TRUE, delimiter = ",")

scene_mbr_acct_dim <- spark_read_csv(sc, 
                                     name='scene_mbr_acct_dim', 
                                     path=paste(in_path_spark, 'scene_mbr_acct_dim.csv', sep=""),
                                     header = TRUE, 
                                     delimiter = ",")

scene_pt_fact <- spark_read_csv(sc, 
                                name='scene_pt_fact', 
                                path=paste(in_path_spark, 'scene_pt_fact.csv', sep=""), 
                                header = TRUE, 
                                delimiter = ",")
...
### NOTE THESE DON'T WORK UNTIL WE COLLECT THE DATA FROM SPARK!
...
h2o.init(nthreads=-1)
h2o_test <- as_h2o_frame(sc, scene_pt_fact, strict_version_check = FALSE)

test_data_set <- h2o_test %>% mutate_if(is.character, as.factor)

test_gervais <- as.factor(h2o_test[,mo_clndr_code])
