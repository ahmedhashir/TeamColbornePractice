...
#Let's create a MAPE function for use later in the model building process
mape<-function(actual, fitted){
     100*mean(
          abs(actual-fitted)/actual)
}

#And make one for Root Mean Squared Error too...
...
#and finally adjusted mape, which will be the mean average error as a percentage of the average actual spend
adj_mape<-function(actual, fitted){
  mae(actual)/(actual-fitted)
}
...
```{r Creating Lagged Variables, echo=FALSE}

#Let's create a time series dataframe that looks only at past purchase behaviour
...
scene_ts<-r_fact%>%
  ...
  right_join(y=scene_ts, by="scene_mbr_key")
...
```{r Selecting Lagged Variables, echo=FALSE}

scene_ts[is.na(scene_ts)]<-0


scene_ts<-scene_ts%>%
  select(one_of("sep_2014",
         "oct_2014",
         "nov_2014", 
         "dec_2014",   
         "jan_2015",     
...
scene_ts<-scene_ts%>% 
  mutate(mean = apply(scene_ts, MARGIN=1, FUN=mean))

scene_ts<-scene_ts%>%
  mutate(sd=apply(scene_ts, MARGIN = 1, FUN=sd))
...
sharpe_hist<-ggplot(scene_ts, aes(Sharpe))+
...
sharpe_scatter<-ggplot(scene_ts, aes(x=sd, y=mean))+
...
nrow(filter(scene_ts, Sharpe>2))/nrow(scene_ts)
...
arima_residuals_erratic<-ggplot(erratic_actuals, aes(x=fitted, y=aug_2016))+
...
arima_residuals_erratic

arima_residuals_consistent<-ggplot(consistent_actuals, aes(x=fitted, y=aug_2016))+
  geom_point(alpha=0.2, color="Blue")+
  coord_cartesian(xlim=c(0,4000),ylim=c(0,4000))+
  labs(x="Fitted Value",
        y= "Actual Spend",
...
arima_residuals_consistent

```


```{r, caret Modelling, echo=FALSE}
...
#First let's merge the demographic data
scene_data <- scene_mbr_dim%>%
                  select(one_of(scene_columns))%>%
              inner_join(y=sharpe, by = "scene_mbr_key")
scene_data[is.na(scene_data)]<-0

train_data_set <- scene_data %>%
  select(
    -scene_mbr_key,
    -scene_mbr_seq_num,
    -eff_from_tmstamp,
...
train_data_set <- train_data_set %>% mutate_if(is.character, as.factor)


library(caret)

#Random Forest stuff from Blue Jays
kfolds=3
search.length=5
data<-train_data_set
...
forest_residuals<-data.frame(cbind(data$aug_2016, forest$finalModel$predicted))
colnames(forest_residuals)<-c("actual", "fitted")

#Mean Absolute Error
MAE <- mean(abs(forest_residuals$actual-forest_residuals$fitted))

#RMSE
rmse(forest_residuals$actual,forest_residuals$fitted)

#Adjusted MAPE
mean(abs(forest_residuals$actual-forest_residuals$fitted))/mean(forest_residuals$actual)

forest_res_plot<-ggplot(forest_residuals, aes(x=fitted))+
...
h2o.init(nthreads = -1)
test_data_set <- as.h2o(test_data_set)
...
```{r, ARIMA Modelling using APPLY, echo=TRUE}
