...
# Aggregating transaction amount by the month, year
...
pt_12mo<-scene_pt_fact%>%
  ...
  group_by(scene_mbr_key)%>%
  summarise("Lag2"=sum(txn_amt))

##Filter for months t-18 to t-13 and aggregate it into one variable
pt_18mo<-scene_pt_fact%>%
  filter(anul_clndr_code == 2015 & mo_clndr_code %in% c("August", "July", "June", "May", "April", "March"))%>%
  group_by(scene_mbr_key)%>%
  summarise("Lag1"=sum(txn_amt))

#Now let's join them into one dataframe
lagged<-left_join(x= sept_spend, y= pt_6mo, by="scene_mbr_key")%>%
                    left_join(y= pt_12mo, by="scene_mbr_key")%>%
                            left_join(y= pt_18mo, by="scene_mbr_key")

colnames(lagged)<-c("scene_mbr_key", "Sept", "Lag1", "Lag2", "Lag3")
...
scene_data <- left_join(x = scene_data, 
              ...
              select(one_of(scene_columns))%>%
                  inner_join(y=lagged, by = "scene_mbr_key")
