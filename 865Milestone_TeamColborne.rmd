...
#Initialize the dataframe that we'll use to create the lagged variables and then chain joins onto:
...
scene_transpose<-t(scene_joined)

#now lets save teh member keys in a new vector and get rid of them for the time series analysis
actuals<-scene_transpose[c(1,"aug_2016)"),]
scene_transpose<-scene_transpose[-1,]

#Let's create a place where we'll store our models

models<-vector("list",length(members))
...
fitted_arima<-as.vector(c(1:756))

for (i in 1:756){
fitted_arima[i]<-as.numeric(models[[i]]$mean)
}
#Let's plot the residuals
plot(scene_joined$aug_2016, fitted_arima)

#Let's calculate the mean absolute error and RMSE
mean(abs(scene_joined$aug_2016-fitted_arima))
rmse(scene_joined$aug_2016, fitted_arima)


#For benchmarking purposes, lets plot how it would look if we just predicted the mean june spend for every customer.

fitted_mean<-rep(mean(scene_joined$jul_2016),length(scene_joined$jul_2016))
head(fitted_mean)

mean(abs(scene_joined$aug_2016-fitted_mean))
rmse(scene_joined$aug_2016, fitted_mean)

#And now let's dive in a bit deeper
