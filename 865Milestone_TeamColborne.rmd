...
scene_loyalty_pt <- scene_loyalty_pt %>%
  group_by(scene_mbr_key, anul_clndr_code, mo_clndr_code) %>%
  summarise(txn_amt = sum(txn_amt), cin_pt_n = sum(ifelse(txn_tp_3=="cin",1, 0)))


head(#Now copy and paste the slightly altered code for each month and join it onto the original dataframe
